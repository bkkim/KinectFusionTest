	...
	namespace cuda {
			...
			float    *out_depth)
		{
			...
			while (true) 
			{
				...
				// Camera to world
				float3 pt_wld;
				pt_wld.x = pt_cam.x - extrinsic.m14;
				pt_wld.y = pt_cam.y - extrinsic.m24;
				pt_wld.z = pt_cam.z - extrinsic.m34;
				pt_wld = extrinsic.getFloat3x3().getTranspose() * pt_wld;
				...
				// zero crossing
				if (tsdf_prev > 0.f && tsdf_curr < 0.f) {
					out_depth[y*width + x] = pt_cam.z + (tsdf_curr*trunc_margin);
					break;
				}
				// free space
				else {
			...
			float    *out_depth)
		{
			...
			depth_raycast_kernel <<< grid, block >>> (
				intrinsic, 
				extrinsic,
				width, 
				height,
