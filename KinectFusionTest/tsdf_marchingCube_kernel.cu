...
void bindVolumeTexture(uchar *d_volume)
{
	...
	checkCudaErrors(cudaBindTexture(0, volumeTex, d_volume, channelDesc));
}
...
#if SAMPLE_VOLUME
	// check the differences between the neighbour fields. 
	// if the diff-values are bigger than dIsoValue, and cubeindex is setted zero.
//	if (field[0] < 0 && field[3] == 1 ||
//		field[1] < 0 && field[2] == 1 || 
//		field[4] < 0 && field[7] == 1 || 
//		field[5] < 0 && field[6] == 1 ||
//		field[4] < 0 && field[0] == 1 ||
...
//		cubeindex = 0;
	...
	if (abs(field[0]) + abs(field[3]) > 1.5 ||
		abs(field[1]) + abs(field[2]) > 1.5 ||
		abs(field[4]) + abs(field[7]) > 1.5 ||
		abs(field[5]) + abs(field[6]) > 1.5 ||
		abs(field[4]) + abs(field[0]) > 1.5 ||
		abs(field[5]) + abs(field[1]) > 1.5 ||
		abs(field[7]) + abs(field[3]) > 1.5 ||
		abs(field[6]) + abs(field[2]) > 1.5)
		cubeindex = 0;
#endif
...
float3 voxelSize, float isoValue, uint activeVoxels, uint maxVerts)
{
	...
	float3 p;
	p.x = gridOrigins.x + (gridPos.x * voxelSize.x);
	p.y = gridOrigins.y + (gridPos.y * voxelSize.y);
	p.z = gridOrigins.z + (gridPos.z * voxelSize.z);
	...
	// check the sum between the neighbour fields. 
	// if the sum-values are bigger than threshold, it is the surface, which is from the inside to the ouside. so, cubeindex is setted zero.
	if (abs(field[0]) + abs(field[3]) > 1.5 ||
		abs(field[1]) + abs(field[2]) > 1.5 ||
		abs(field[4]) + abs(field[7]) > 1.5 ||
		abs(field[5]) + abs(field[6]) > 1.5 ||
...
float3 voxelSize, float isoValue, uint activeVoxels, uint maxVerts)
{
	generateTriangles2 <<< grid, NTHREADS >>>(pos, norm,
		...
		gridSize, gridSizeShift, gridSizeMask, gridOrigins,
		voxelSize, isoValue, activeVoxels,
		maxVerts);
